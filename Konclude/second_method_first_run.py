...
#Function to check if a class in inconsistant
def check2():
    save()
    ...
    with open('result.txt', 'r') as file:
        for line in file:
            if line[0:5]=="false":
                return True
    return False
#Function to delete inconsistant classes
def check(el,label):
    save()
    os.system('start /wait cmd /c "Konclude satisfiability -i Test.owl -o result.txt -x '+el.iri+'"')
    with open('result.txt', 'r') as file:
...
for i in range(len(SubClasses)):
    if classes[i] in total1 or  classes[i] in total2 or  classes[i] in total3 or  classes[i] in total4 or  classes[i] in total5:
        continue
    if SubClasses[i]==[]:
        ...
        if classes[i] not in total1 and classes[i] not in total2 and classes[i] not in total3 and classes[i] not in total4 and classes[i] not in total5:
            ...
            if classes[i] in total5:
                exec(f"with onto: \n  class {class_name}({subclasses_of_el}): \n    label=\"{label}\" \n    namespace=a5 ")
#deleting again inconsistant classes
"""
all_classes=list(onto.classes())
for el in all_classes:
    os.system('start /wait cmd /c "Konclude satisfiability -i Test.owl -o result.txt -x '+el.iri+'"')
    with open('result.txt', 'r') as file:
        for line in file:
            if line[0:5]=="false" and el.iri not in total:
...
for el in data1:
    ...
    if el["type"]!="subclass of":
        ...
        #Resoning
        if el["head"] not in total1 and el["head"] not in total2 and el["head"] not in total3 and el["head"] not in total4 and el["head"] not in total5:
            exec(f"with onto: \n  Entity={onto_head}.{head}")
            check(Entity,el["head"])
        if el["tail"] not in total1 and el["tail"] not in total2 and el["tail"] not in total3 and el["tail"] not in total4 and el["tail"] not in total5:
            exec(f"with onto: \n  Entity={onto_tail}.{tail}")
            check(Entity,el["tail"])
        if (el["head"]  in total1 or el["head"]  in total2 or el["head"]  in total3 or el["head"]  in total4 or el["head"]  in total5) and (el["tail"]  in total1 or el["tail"]  in total2 or el["tail"]  in total3 or el["tail"]  in total4 or el["tail"]  in total5):
            if check1(Entity,el["head"]) or check1(Entity,el["tail"]):
                exec(f"{onto_head}.{head}.is_a.remove({onto_rel}.{relation}.some({onto_tail}.{tail}))")
                print("removed a relation")
...
for el in instanceof:
    ...
    exec(f"with onto: \n  instance1={onto_head}.{changed_head}(\"{head}\")")
    #reasoning
    if check2():
        print("y")
        with onto:
